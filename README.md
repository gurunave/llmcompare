...
speed and context, for 139 models across 29 providers — 89 of them open-weights,
...
| `/models/[id]` | One page per model — headline stats, its own radar and benchmark bars, full specs, and the closest alternatives by price and capability. 139 statically generated pages. |
...
`qwen3-8-max-0902` (added 2026-09-08) is Alibaba's September 2 post-training
refresh of `qwen3-8-max` — same architecture, context and pricing per
third-party aggregators, since Alibaba's own model card was unreachable from
this environment. Its reported gains are on evals (Code Arena WebDev,
Terminal-Bench 3.0, ProgramBench) outside this catalog's benchmark set, and
the aggregator-reported GPQA/SWE-bench Pro/OSWorld figures were identical to
the base model's, so scores were left blank rather than assumed unchanged.
Fill them in once a primary source is reachable.
